...
def add_estimated_computation(total_df):
    ...
    for index, row in total_df.iterrows():
        ...
        if 'comp_coeff' in total_df.columns:
            row_comp_coeff = row['comp_coeff']
        else:
            row_comp_coeff = [1 for a in range(len(row_placement))]

        if 'image_size_coeff' in total_df.columns:
            row_image_size_coeff = row['image_size_coeff']
        else:
            row_image_size_coeff = 1
        ...
        # print("^^^^ ", pickle_latency, "   ", row_image_size_coeff)
        pickle_comp_est_er.append(pickle_latency*row_image_size_coeff)
        ...
        for i, (curr_partition, curr_placement) in enumerate(zip(row['partitioning'], row['placement'])):
            ...
            dnn_comp_partitionwise_plus_est_er.append(dnn_latency*(estimated_stayrate_list[i]+estimated_exitrate_list[i])*row_comp_coeff[i]*row_image_size_coeff)
            ...
            if i != len(row['placement'])-1:
                ...
                pickle_comp_est_er.append(pickle_latency*estimated_stayrate_list[i]*row_image_size_coeff)
...
def add_estimated_pickle_size(total_df):
    ...
    for index, row in total_df.iterrows():
        ...
        df_pickle_size.append(np.array(pickled_size)*row_image_size_coeff)
