...
plt.scatter(latency_constraint_list[next((i for i, x in enumerate(cloud_accuracy_list) if x), None)], cloud_accuracy_list[next((i for i, x in enumerate(cloud_accuracy_list) if x), None)], marker='o', facecolor="none", edgecolors="blue", linewidths=2, s=150, color='darkblue', label='cloud-only', zorder=10)
