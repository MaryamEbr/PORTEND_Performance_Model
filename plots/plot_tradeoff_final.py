...
rows = total_df.loc[(total_df['ruined_flag']==False)&(total_df['destination_tier']==destination_tier) & (total_df['model_mode']==model_mode) &(total_df['e2e_tail_mean']<175)]

#### to only show the points closer to pareto frontier
rows = rows.drop(rows[(rows['e2e_tail_mean']>50) & (rows['accuracy']<0.91)].index)

rows_2p_ns = rows.loc[ (rows['partitioning'].astype(str)=='[[1, 1], [2, 55]]')]
...
plt.plot(all_tier_latency_ee, all_tier_accuracy_ee, 'o', color='orange', label='1, 2, 3 partitions, with early exit (our method)')
