...
experiment_comm_latency = np.array(rows[['sum_communications_tail_mean']].to_numpy()[:,0])

estimated_comm_latency = np.array(rows[['estimated_communication_latency']].to_numpy()[:,0])



fig,ax = plt.subplots(figsize=(9,6))
plt.scatter(experiment_comm_latency, estimated_comm_latency, color='purple')
plt.plot(np.arange(0, 2000, 0.01), np.arange(0, 2000,0.01), color='black')
...
plt.xlabel('experiment communication latency')
plt.ylabel('estimated communication latency')
