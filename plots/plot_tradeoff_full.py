...
rows = total_df.loc[(total_df['ruined_flag']==False) & (total_df['accuracy']>0.60) & 
...
# our method
all_tier_latency_ee = rows.loc[()]['e2e_tail_mean'].values
all_tier_accuracy_ee = rows.loc[() ]['accuracy'].values
...
plt.plot(all_tier_latency_ee, all_tier_accuracy_ee, 'o', color='orange', label='1, 2 partitions, with early exit (our method)')

plt.plot(all_tier_0_1_latency_ns, all_tier_0_1_accuracy_ns, 'X', ms=10, color='purple', label='2 partitions, no early exit (neurosurgeon)')
plt.plot(all_tier_0_2_latency_ns, all_tier_0_2_accuracy_ns, 'X', ms=10, color='purple', )

plt.title(f"model {model_name}(full config model), dataset {dataset}")
...
plt.bar(2, all_tier_1_comm_latency, width=0.3,color='orange')
plt.bar(2, all_tier_1_comp_latency, bottom=all_tier_1_comm_latency ,width=0.3,color='brown')

plt.bar(3, all_tier_2_comm_latency, width=0.3,color='orange')
plt.bar(3, all_tier_2_comp_latency, bottom=all_tier_2_comm_latency,  width=0.3,color='brown')
